...
export default function List ({ people ,userPoint }:{people:IPeople[]|undefined, userPoint:IPoint|undefined }) { 
  ...
  return (
      ...
      people &&  userPoint?
      <div className='h-auto w-full mt-5 px-5 grid gap-4 grid-cols-1 xl:grid-cols-2'>
        {people.sort((a,b)=>a.distance - b.distance).map(p=><People people={p} key={p.name}  onClick={openWindowById}/>)}
      </div>
      :
      <div className={`mt-10 text-4xl font-bold my-10 ${light? 'text-black':'text-yellow-500'}`}>Click map to point your current location</div>
